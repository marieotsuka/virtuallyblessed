...
function createSmoke(){
  ...
  smokePositions = new Float32Array(totalPointCount * 3); // smokeCount * 3
  ...
  // smokeParticles.scale.multiplyScalar(0.001);//0.001
  ...
  let smokeSettle = new TWEEN.Tween(smokeParticles.scale)
      .to({x: 0.4, y: 0.4, z: 0.4}, 500)
      .easing(TWEEN.Easing.Cubic.InOut)

  
  // smokeAppear.chain(smokeScale);
  // smokeScale.chain(smokeExpand);
  // smokeExpand.chain(smokeSettle);
  // smokeAppear.start();
...
function createLogo(){
  ...
  loader.load(
    ...
    function ( data ) {
      ...
      vbLogo.position.set(0, 0, 0);
      
      sphereGroup.add( vbLogo );
      totalPointCount = logoAllPoints.length;
    },      
  ...
  let logoScaleTarget = new THREE.Vector3(0.5,-0.5,0.5);

  let logoPosTarget = new THREE.Vector3(-2.75, 3, 0);
  setTimeout(function(){
  ...
  for ( let i = 0; i < vbLogo.children.length; i ++ ) {
      ...
      if ( logoPoints instanceof THREE.Points ) {
        ...
        for( let j = 0; j < count; j ++ ) {
            ...
            logopt_alphas.array[j] = 0.1 + Math.sin( 1 * j + time ) ;
            logopt_sizes.array[j] = 0.8 * ( 2 + Math.sin( 5 * j + time ) );
...
function updateParticleAttributes(position, size) {
    ...
    // for(var v =0; v<logoAllPoints.length; v++){
    //   smokeParts[v * 3 + 0] = logoAllPoints[v].x;
    //   smokeParts[v * 3 + 1] = logoAllPoints[v].y;
    //   smokeParts[v * 3 + 2] = logoAllPoints[v].z;
    // }
...
function animateSmoke(time){
  ...
  let timedif = 0.00006*(time-start);
  ...
  // smokeParticles.rotation.y = (0.0001) * time;
}
